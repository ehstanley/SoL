...
no3_yr <- SoLno2no3 %>% group_by(lagoslakeid) %>% summarise(n_yrs = n_distinct(year))
...
state_areas <- aggregate(lakes$lake_area_ha, by = list(lakes$state_name), FUN = median)
areas2 <- aggregate(lakes$lake_area_ha, by = list(lakes$state_name), FUN = mean)

mn <- lakes[which(lakes$state_name == "Minnesota"),]
mn2 <- sol[which(sol$state_name == "Minnesota"),]
